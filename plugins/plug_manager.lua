...
local function run(msg, matches) 
    ...
    if (matches[1] == "giv" or matches[1] == "جيب ملف") and is_sudo(msg) then 
    if not we_sudo(msg) then return "🍥￤هذا الاوامر للمطور الاساسي فقط 🛠" end
    ...
    if (matches[1] == "del" or matches[1] == "حذف ملف")  and matches[2] and is_sudo(msg) then 
    if not we_sudo(msg) then return "🍥￤هذا الاوامر للمطور الاساسي فقط 🛠" end
    ...
    if (matches[1] == 'حدث نفسك' or matches[1] == 'حدث نفسك') and is_sudo(msg) then
    if not we_sudo(msg)  then return "📛*￤* هذا الاوامر للمطور الاساسي فقط 🛠" end
    return"👨🏾‍🔧*¦* انتهى دعم هذا السورس بالتحديثات ترقبو السورس الجديد وبالتيجي الجديد في قناة الزعيم @vb_nett . \n📦"
    end
    
    if (matches[1] == 'سيرفر' or matches[1]:lower() == 'server') and is_sudo(msg) then
    return io.popen([[
